...
class TestGuiderImage(guiderTester.GuiderTester, unittest.TestCase):
    def setUp(self):
        self.verbose = True
        # calibration files
        self.inDarkFile, self.outDarkFile = _inout_names('gcam', 57357,
                                                         'gimg-0001.fits.gz')
        self.inFlatFile, self.outFlatFile = _inout_names('gcam', 57357,
                                                         'gimg-0003.fits.gz')
        self.inFlatEcamFile, self.outFlatEcamFile = _inout_names(
            'ecam', 57254, 'gimg-0003.fits.gz')

        # bad files
        self.saturatedFile = guiderTester.getTestImage(
            'gcam', 56587, 'gimg-0004.fits.gz')
        self.badReadFile1 = guiderTester.getTestImage(
            'gcam', 56608, 'gimg-0859.fits.gz')
        self.badReadFile2 = guiderTester.getTestImage(
            'gcam', 56331, 'gimg-0519.fits.gz')

        # good data files
        self.inDataFile, self.outDataFile = _inout_names(
            'gcam', 57357, 'gimg-0040.fits.gz')

        # values for comparison after processing good data files.
        self.flatExpect = guiderTester.getTestImage(
            'gcam', 57357, 'expect-gimg-0003.fits.gz')
        self.dataExpect = guiderTester.getTestImage(
            'gcam', 57357, 'expect-gimg-0040.fits.gz')
