...
from guiderActor.Commands.GuiderCmd import (GuiderCmd, GuiderState, getGprobeKeys, getGuideInfoKey,
                                            yanny)
...
def getGprobes(plate, pointing, fiberPos):
    ...
    pmFilePath = os.path.join(
        plPlugMapDir, 'plPlugMapP-%i%s.par' %
        (plate, '' if pointing == 'A' else pointing))
