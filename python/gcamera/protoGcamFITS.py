...
class PullTest(object):
    ...
    def getPlugmap(self, plugmapfile):
        pm = YPF.YPFreadone(plugmapfile, 'PLUGMAPOBJ')
        w = pm.holeType == 'GUIDE'
        pm = pm[w]
        self.plugmap = pm
    ...
    def ISR(self):
        """ Dummy ISR: 'bias subtracts' the median pixel. """
        
        sub = np.median(self.rawImage)
        self.cleanImage = (self.rawImage - sub).astype('int16')
