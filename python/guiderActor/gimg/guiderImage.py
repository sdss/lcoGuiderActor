...
class GuiderImageAnalysis(object):
    ...
    def fillPrimaryHDU(self, cmd, models, imageHDU, gState, frameInfo, objectname):
        ...
        try:
            ...
            imageHDU.header['GCAMSCAL'] = (
                frameInfo.guideCameraScale,
                'guide camera plate scale (mm/pixel)')
            imageHDU.header['PLATSCAL'] = (
                frameInfo.plugPlateScale,
            ...
            self.cmd.error(
                'text=%s' %
                qstr(
                    '!!!!! failed to fill out primary HDU  !!!!! (%s)' %
                    (e)))
            raise e
        ...
        for name, fitsName, comment in defs:
            try:
                ...
                self.cmd.warn(
                    'text=%s' %
                    qstr(
                        'failed to make guider card %s=%s (%s)' %
                        (name, val, e)))
    ...
    def rotate_one_fiber(self, image, mask, fiber, r):
        ...
        rmaskstamp = interpolation.rotate(
            mask[stamp_slice], -rot, cval=self.mask_masked, reshape=False, order=0)
    ...
    def _get_basic_hdulist(self, image, primhdr, bg):
        ...
        imageHDU.header['SDSSFMT'] = (
            '{}PROC 1 4'.format(self.camera[0].upper()),
    ...
    def _getProcGimgHDUList(self, primhdr, gprobes, fibers, image, mask):
        ...
        try:
            ...
            self.cmd.warn(
                'text=%s' %
                qstr(
                    'could not create header for proc- guider file: %s' %
                    (e,)))
        ...
        try:
            if self.camera == 'gcamera':
                hdulist = self._getProcGimgHDUList(
                    hdr, gprobes, self.fibers, image, self.maskImage)
                doCompress = True
            ...
            actorFits.writeFits(cmd, hdulist, directory, filename,
                                doCompress=doCompress, chmod=0o644,
    ...
    def _check_ccd_temp(self, header):
        """Checks if the gcamera CCDTEMP is within deltaTemp of setPoint and darkTemperature."""
    ...
    def _pre_process(self, filename, binning=1):
        ...
        if nSat > 4000:
            self.cmd.error('text=%s' % qstr('Fully saturated! Please reduce exposure '
                                            'time or wait for the excess light to go away.'))
    ...
    def _set_fiber_star(self, fiber, stars, image, mask, stampFrameCoords):
        ...
        try:
            ...
            self.cmd.warn(
                'text=%s' %
                qstr(
                    'No star found via PyGuide.findStars for fiber %d.' %
                    (fiber.fiberid)))
            return

        if not star.isOK:
            self.cmd.warn(
                'text=%s' % qstr('Danger: this should never happen! '
                                 'PyGuide.findStars failed on fiber %d with: %s.' %
                                 (fiber.fiberid, star.msgStr)))
            return
        ...
        try:
            ...
            if not shape.isOK:
                self.cmd.warn(
                    'text=%s' %
                    qstr(
                        'PyGuide.starShape failed on fiber %d with: %s.' %
                        (fiber.fiberid, star.msgStr)))
                return
            ...
            self.cmd.warn(
                'text=%s' %
                qstr(
                    'PyGuide.starShape failed on fiber %d with Exception: %s.' %
                    (fiber.fiberid, e)))
    ...
    def findStars(self, gprobes):
        ...
        self.cmd.diag(
            'text=%s' %
            qstr(
                'After flattening: image range: %g to %g' %
                (image.min(), image.max())))
    ...
    def applyDark(self, image, darkFileName):
        ...
        # Create and process the dark image if this is the first time through, or
        # a new dark exposure
    ...
    def analyzeDark(self, darkFileName, cmd=None, setPoint=None):
        ...
        if expdate > pre_stacking_date and (stack < 3 or exptimen < 30):
            errMsg = ('Total dark exposure time too short: '
                      'minimum stack of 3 with total time > 30s.')
        ...
        actorFits.writeFits(cmd, hdu, directory, filename, doCompress=True, chmod=0o644)
    ...
    def _find_fibers_in_flat(self, image, flatFileName, gprobes, hdr, force=False):
        ...
        for f in fibers:
            ...
            if dr < 0.25:
                ...
                self.cmd.inform(
                    'text=%s' % qstr('Rejecting fiber at (%i,%i) in unbinned pixel coords, '
        ...
        self.cmd.inform(
            'text=%s' %
            qstr(
                'Matched %i fibers, with dx,dy = (%g,%g)' %
                (len(fmap), dx, dy)))
        ...
        for i, f in enumerate(fibers):
            if i not in fmap:
                self.cmd.diag(
                    'text=%s' %
                    qstr(
                        'probe %d (%d) not matched; skipping...' %
                        (i, f.fiberid)))
                continue
            f.fiberid = fmap[i]
    ...
    def analyzeFlat(self, flatFileName, gprobes, cmd=None, setPoint=None, force=False):
        ...
        actorFits.writeFits(cmd, hdulist, directory, filename, doCompress=True, chmod=0o644)
